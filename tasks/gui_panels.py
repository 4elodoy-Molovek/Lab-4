...
class ResultView:
    ...
    def _metric_rows(self, d, is_test):
        rows = [
            ...
            ("Норма шага ‖v^(N) − v^(N−1)‖", f"{d['eps_N']:.6e}"),
            ("Спектральный радиус метода ρ", f"{d['rho_est']:.6f}"),
            ("Апостериорная оценка ε метода", f"{d['eps_apost']:.6e}"),
    ...
    def show_result_popup(self, d, is_test, method):
        if is_test:
            text = (
                ...
                f"Норма шага ‖v^(N) − v^(N−1)‖: {d['eps_N']:.4e}\n"
                f"Спектральный радиус метода ρ: {d['rho_est']:.6f}\n"
                f"Апостериорная оценка ε метода: {d['eps_apost']:.4e}\n"
